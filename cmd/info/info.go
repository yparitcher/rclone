...
func (r *results) writeFile(path string) (fs.Object, error) {
	...
	obj, err := r.f.Put(bytes.NewBufferString(contents), src)
	if uploadWait > 0 {
...
func (r *results) checkStringPositions(k, s string) {
	...
	for _, pos := range internal.PositionList {
		...
		obj, getError := r.f.NewObject(path)
		if getError != nil {
			res.GetError[pos] = getError.Error()
			fs.Infof(r.f, "Getting %s position file 0x%0X Error: %s", pos.String(), s, getError)
		} else {
			if obj.Size() != 50 {
		...
		if writeError != nil || getError != nil {
			positionError += pos
		}
	}
...
func (r *results) checkControls() {
	...
	for i := rune(0); i < 128; i++ {
		...
		go func(s string) {
			...
			token := <-tokens
			k := strconv.Quote(s)[1:]
			r.checkStringPositions(k[:len(k)-1], s)
			tokens <- token
		}(s)
	}
